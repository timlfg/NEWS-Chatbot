# analysis2.py — görev bazlı (SUMM/QA) analiz + epoch grafikleri (yeni isimlendirme)
...
if os.path.exists(METRICS_CSV):
    ...
    make_lineplot(x, [df["train_loss"].values, df["eval_loss"].values],
                  ...
                  "Loss (train vs val)", os.path.join(OUTPUT_DIR,"loss_curves_new.png"))

    make_lineplot(x, [df["train_token_f1"].values, df["eval_token_f1"].values],
                  ["train_token_f1","val_token_f1"], "step","token_f1",
                  "Token F1 (train vs val)", os.path.join(OUTPUT_DIR,"tokenf1_curves_new.png"))

    make_lineplot(x, [df["train_em"].values, df["eval_em"].values],
                  ["train_EM","val_EM"], "step","EM",
                  "Accuracy (Exact Match)", os.path.join(OUTPUT_DIR,"accuracy_curves_em_new.png"))
    ...
    if "epoch" in df.columns and df["epoch"].notna().any():
        ...
        make_lineplot(g["epoch_i"].values, [g["train_loss"].values, g["eval_loss"].values],
                      ...
                      os.path.join(OUTPUT_DIR,"loss_curves_by_epoch_new.png"))
else:
    print(f"[WARN] metrics.csv yok: {METRICS_CSV}")

# ---------- 2) eval_predictions.jsonl ----------
...
if os.path.exists(EVAL_PRED_JSONL):
    ...
    if summ_rouges:
        ...
        out=os.path.join(OUTPUT_DIR,"summ_rouge_hist_new.png")
        plt.savefig(out,dpi=150); plt.close(); print(f"[INFO] saved {out}")

    if qa_em:
        y_true=np.array(qa_em,int); y_score=np.array(qa_f1,float)
        thr=0.5; y_pred=(y_score>=thr).astype(int)
        cm=confusion_matrix_binary(y_true,y_pred)
        plot_confusion_matrix(cm,["EM=0","EM=1"],
                              f"Confusion Matrix (QA, thr={thr})",
                              os.path.join(OUTPUT_DIR,"qa_confusion_matrix_new.png"))
        ...
        out=os.path.join(OUTPUT_DIR,"qa_roc_curve_new.png")
        plt.tight_layout(); plt.savefig(out,dpi=150); plt.close(); print(f"[INFO] saved {out}")
        ...
        out=os.path.join(OUTPUT_DIR,"qa_tokenf1_hist_new.png")
        plt.savefig(out,dpi=150); plt.close(); print(f"[INFO] saved {out}")
else:
    print(f"[WARN] eval_predictions.jsonl yok: {EVAL_PRED_JSONL}")

print("[DONE] Analiz bitti. (_new dosyalar üretildi)")
